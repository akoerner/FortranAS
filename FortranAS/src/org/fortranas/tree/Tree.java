...
/*
    public static void promote(Map<String, Object> node, Map<String, Object> parentRule, Map<String, Object> childRule) {
        if (node.containsKey("children") && node.get("children") instanceof List) {
            @SuppressWarnings("unchecked")
            List<Map<String, Object>> children = (List<Map<String, Object>>) node.get("children");

            for (Map<String, Object> child : children) {
                promote(child, parentRule, childRule);
            }

            System.out.println("checking parent: " + node.get("text") + " rule: " + parentRule.get("tokenType"));
            if (matchesRule(node, parentRule)) {
                System.out.println("Parent matches: " + node.get("text"));
                for (Map<String, Object> child : children) {
                    System.out.println("checking child: " + child.get("text") + " rule: " + childRule.get("tokenType"));
                    if (matchesRule(child, childRule)) {
                        System.out.println("Child matches: " + child.get("text"));
                        System.out.println("!!!Promoting: " + child.get("text"));
                        Tree.clearNode(node);
                        //Tree.copyData(node, child);
                        node.putAll(Maps.difference(child, node).entriesOnlyOnLeft()); 
                        break;
                    }
...
*/
    ...
    public static boolean wildcardCompare__(String wildcard, String str) {
        ...
        for (String part : parts) {
            int index = str.indexOf(part, lastIndex);
            if (index < lastIndex) {
                return false;
            }
    ...
    public static boolean wildcardCompare_(String wildcard, String str) {
        String regex = wildcard.replaceAll("[.*+?^${}()|\\[\\]\\\\]", "\\\\$0").replace("*", ".*");

        Pattern pattern = Pattern.compile(regex);

        Matcher matcher = pattern.matcher(str);

        System.out.println(str + " matches " + wildcard + ":" + matcher.matches());
